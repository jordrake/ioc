...
test("registerClass should return the same instance of the class on consecutive gets if singleton option is true", (t) => {
    t.plan(1);
    const StubClass = class {
    };
    const container = new container_1.default();
...
test("execute should have specified dependencies injected into it", (t) => {
    ...
    container.execute(["hello", "foo", "stubClass"], (hello, foo, stubClass) => {
        t.equal(hello, "world");
        t.equal(foo, "bar");
